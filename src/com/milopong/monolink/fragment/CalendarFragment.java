...
public class CalendarFragment extends Fragment implements
		...
		WeekView.EventLongPressListener, OnClickListener {
	private static final int TYPE_DAY_VIEW = 1;
	private int mWeekViewType = TYPE_DAY_VIEW;
	private WeekView mWeekView;
	private ArrayList<Schedule> schedules;
	private final String eventColor[] = { "DC6E6E", "#A875B2", "#71A0D4",
			"#7CC9A6", "#ADD371", "#DBDB71", "#BF76B1" };

	public CalendarFragment(int mode, ArrayList<Schedule> schedules) {
		this.schedules = new ArrayList<Schedule>();
		this.schedules = schedules;
		this.mWeekViewType = mode;
	}
	...
	public List<WeekViewEvent> onMonthChange(int newYear, int newMonth) {
		...
		try {
			...
			for (int i = 0; i < schedules.size(); i++) {
				...
				startTime.setTime(formatter.parse(schedules.get(i)
						.getStartTime()));
				endTime = (Calendar) startTime.clone();
				endTime.setTime(formatter.parse(schedules.get(i).getEndTime()));

				WeekViewEvent event = new WeekViewEvent(schedules.get(i)
						.getNo(), schedules.get(i).getName(), startTime,
						endTime);
